...
// Create a placeholder card if necessary
if (modules.length % 3 === 2) {
    modules.push({
...
// Function to format the date and include the day name
function formatDateWithDay(dateString) {
    const date = new Date(dateString);
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
    return date.toLocaleDateString(undefined, options); // Locale undefined to use user's locale
}

// Function to generate cards dynamically
function generateCards(filteredModules) {
    ...
    filteredModules.forEach(module => {
        ...
        // Grey out the module if it's locked
        if (currentDate < moduleOpenDate) {
            card.classList.add('locked');
        }

        // If it's a placeholder, make it invisible
        if (module.moduleNumber === 'Placeholder') {
            card.classList.add('invisible'); // Add a class for invisible cards
        } else {
            card.innerHTML = `
            <img src="${module.moduleImage}" alt="${module.moduleTitle} Image" class="module-image">
            <div class="module-info">
                <div class="module-number">${module.moduleNumber}</div>
                <div class="module-title">${module.moduleTitle}</div>
            </div>
            <div class="assistant-info">
                <img src="${module.assistantPic}" alt="Assistant Profile" class="assistant-pic">
                <div>
                    <div class="assistant-name">${module.assistantName}</div>
                    <div class="assistant-id">${module.assistantId}</div>
                </div>
            </div>
        `;
        }

        // Add click event listener to check date
        card.addEventListener('click', () => {
            if (currentDate >= moduleOpenDate) {
                // Open the module page if the date is reached
                window.location.href = `../modules/${modules.indexOf(module)+1}`;
            } else {
                // Show a popup with the day name if the module is not yet open
                const formattedDate = formatDateWithDay(module.openDate);
                showModal(`This module will open on:\n\n${formattedDate}\n\nPlease come back later.`);
            }
        });
...
generateCards(modules);

// Function to search and filter modules
function searchModules() {
    const searchInput = document.getElementById('search-input').value.toLowerCase();
    
    const filteredModules = modules.filter(module => {
        return module.moduleTitle.toLowerCase().includes(searchInput) || 
               module.moduleNumber.toLowerCase().includes(searchInput);
    });
    
    generateCards(filteredModules);
}
